...
class PdeModel:
    ...
    def Pde_residual(self, x, y, training=True):
        ...
        with tf.GradientTape(persistent=True) as tape:
            tape.watch([x, y])
            #  forward pass
            u, v, p = self.nn_model([x, y], training=training)
        ...
        # momentum equations
        fx = u * ux + v * uy + px - self.nue * (uxx + uyy)
        fy = u * vx + v * vy + py - self.nue * (vxx + vyy)
    ...
    def run(self, epochs, log_dir, wb=False, verbose_freq=1000, plot_freq=10000):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % plot_freq == 0:
                ...
                # self.get_plots(epoch + 1, log_dir=log_dir, data_dir=data_dir, grid_dir=grid_dir,
                # top_velocity=tv, wb=wb)
